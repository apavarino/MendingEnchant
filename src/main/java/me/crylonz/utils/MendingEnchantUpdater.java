...
public class MendingEnchantUpdater {
    ...
    private MendingEnchantUpdater.UpdateResult result = MendingEnchantUpdater.UpdateResult.SUCCESS;
     ...
     */
    public MendingEnchantUpdater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
     ...
     */
    public MendingEnchantUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback) {
     ...
     * @param announce True if the program should announce the progress of new updates in console.
     */
    public MendingEnchantUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
        this.plugin = plugin;
        this.type = type;
        ...
        try {
            this.url = new URL(MendingEnchantUpdater.HOST + MendingEnchantUpdater.QUERY + this.id);
     ...
     * @see UpdateResult
     */
    public MendingEnchantUpdater.UpdateResult getResult() {
        this.waitForThread();
        return this.result;
    }
...
//            fout = new FileOutputStream(new File(this.updateFolder, file.getName()));
            ...
            final byte[] data = new byte[MendingEnchantUpdater.BYTE_SIZE];
            int count;
            if (this.announce) {
                this.plugin.getLogger().info("About to download a new update: " + this.versionName);
            }
            long downloaded = 0;
            while ((count = in.read(data, 0, MendingEnchantUpdater.BYTE_SIZE)) != -1) {
                downloaded += count;
            ...
            this.result = MendingEnchantUpdater.UpdateResult.FAIL_DOWNLOAD;
        } finally {
            try {
                if (in != null) {
                    in.close();
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (!entry.isDirectory()) {
                    ...
                    int b;
                    final byte[] buffer = new byte[MendingEnchantUpdater.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, MendingEnchantUpdater.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, MendingEnchantUpdater.BYTE_SIZE)) != -1) {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(DELIMETER).length >= 2) {
                ...
                if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion)) {
                    ...
                    this.result = MendingEnchantUpdater.UpdateResult.NO_UPDATE;
                    return false;
                }
            } else {
                ...
                this.result = MendingEnchantUpdater.UpdateResult.FAIL_NOVERSION;
                return false;
            }
        }
        return true;
    ...
    private boolean hasTag(String version) {
        for (final String string : MendingEnchantUpdater.NO_UPDATE_TAG) {
    ...
    private boolean read() {
        try {
            ...
            conn.addRequestProperty("User-Agent", MendingEnchantUpdater.USER_AGENT);
            ...
            this.versionName = (String) latestUpdate.get(MendingEnchantUpdater.TITLE_VALUE);
            this.versionLink = (String) latestUpdate.get(MendingEnchantUpdater.LINK_VALUE);
            this.versionType = (String) latestUpdate.get(MendingEnchantUpdater.TYPE_VALUE);
            this.versionType = (String) latestUpdate.get(MendingEnchantUpdater.TYPE_VALUE);
            this.versionGameVersion = (String) latestUpdate.get(MendingEnchantUpdater.VERSION_VALUE);
            this.versionFileName = (String) latestUpdate.get(MendingEnchantUpdater.VERSION_FILENAME);
    ...
    public interface UpdateCallback {
         ...
         */
        void onFinish(MendingEnchantUpdater updater);
    }
